...
const CashFlowPage = () => {
  ...
  const handleSubmitWithdraw = async () => {
    const result = await axios.post("/withdraws/withdraw", {
  ...
  const convertDate = (stringDate) => {
    ...
    const day =
      dateObj.getDay() < 10 ? `0${dateObj.getDay()}` : dateObj.getDay();
    const month =
      dateObj.getMonth() < 9
  ...
  useEffect(() => {
    const getUser = async () => {
      const result = await axios.get("/users/user/active", {
        headers: {
    ...
    const getTotalWithdraw = async () => {
      const result = await axios.get("/withdraws/withdraw");
      console.log(result, "Total Withdraw");
      setTotalWithdraw(result.data);
    };
    const getTotalDeposit = async () => {
      const result = await axios.get("/deposits/deposit");
          ...
          {/* {withdraws.map((withdraw) => (
            ...
            >
              ...
              <div className="text-right">
                R${" "}
                {withdraw.amount.toLocaleString("pt-br", {
                  minimumFractionDigits: 2,
                })}
              </div>
            </div>
        ...
        <section className="mb-3">
          ...
          {deposits.map((deposit) => (
            ...
            >
              ...
              <div className="text-right">
                R${" "}
                {deposit.amount.toLocaleString("pt-br", {
                  minimumFractionDigits: 2,
                })}
              </div>
            </div>
