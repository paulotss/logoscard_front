...
const BenefitsPage = () => {
  ...
  const handleButtonUsageBenefit = async () => {
    ...
    if (assignmentBenefit.benefit.amount + nDependents > Number(amount)) {
      try {
        const result = await axios.put("/benefits/assignment/benefit", {
  ...
  useEffect(() => {
    const getUser = async () => {
      setIsLoading(true);
      try {
        const result = await axios(`/users/user/${id}`);
        setUser(result.data);
          ...
          <main className="p-5">
                    ...
                    <div key={ab.benefit.id}>
                      ...
                      >
                        ...
                        <div className="text-right">
                          {ab.benefit.amount +
                            user.assignments[0].dependents.length}
                        </div>
                        <div className="text-right flex justify-end">
                          <button
                            ...
                            onClick={() =>
                              handleOpenUsege(ab.benefitId, ab.amount)
                            }
                            // value={ab.amount}
                            // id={ab.benefitId}
                          >
            ...
            <Dialog open={openUsage} onClose={handleCloseUsage}>
              ...
              <DialogActions>
                ...
                <Button variant="contained" onClick={handleCloseUsage}>
                  Cancelar
                </Button>
              </DialogActions>
            </Dialog>
