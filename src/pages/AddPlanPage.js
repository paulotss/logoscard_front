...
const AddPlanPage = () => {
  ...
  const submitPlanForm = async () => {
    setIsLoading(true);
    try {
      const newUser = await axios.post("/assignments/assignment", {
      ...
      await axios.post(
        "/benefits/assignment/benefit",
        getPlanBenefits(newUser.data)
      );
      if (dependents.length > 0) {
        ...
        console.log(dependentsData);
        await axios.post("/users/user/dependent", dependentsData);
      }
  ...
  const handleChange = ({ target }) => {
    ...
    if (name === "planId") {
      ...
      const newFee =
        value > 1 ? (getPriceById(activePlan.planId) * 10) / 100 : 0;
      setActivePlan({
        ...activePlan,
        [name]: value,
  ...
  useEffect(() => {
    const getPlans = async () => {
      setIsLoading(true);
      try {
        const result = await axios.get("/plans/plans");
            ...
            <div className="flex">
              <form className="mb-3 w-fit">
                <div className="flex flex-wrap">
                  ...
                  <div className="p-2 border rounded-md m-2 w-80 h-fit">
                    <label htmlFor="parcels" className="text-sm">
                      Parcelas
                    </label>
                    <br />
                    <select
                  ...
                  <div className="p-2 border rounded-md m-2 w-80 h-fit">
                    <p className="text-sm">Valor do plano</p>
                    <p className="font-bold text-2xl">
                      R${" "}
                      {(activePlan.price + activePlan.fees).toLocaleString(
                        "pt-br",
              ...
              <aside className="w-96 border rounded bg-white p-5 h-fit">
                <div className="flex justify-between">
                  ...
                  <div className="text-2xl font-bold">
                    + R${" "}
                    {plans
                      .find((plan) => plan.id === Number(activePlan.planId))
                ...
                <div className="flex justify-between">
                  ...
                  <div className="text-2xl font-bold">
                    + R${" "}
                    {activePlan.fees.toLocaleString("pt-br", {
                      minimumFractionDigits: 2,
                    })}
                  </div>
                </div>
