...
const ClientsListPage = () => {
  ...
  const extractNameDependents = (dependents) => {
    const result = dependents.reduce(
      (acc, dependent) =>
        (acc +=
          `${dependent.user.firstName} ${dependent.user.lastName}`.toLowerCase()),
      ""
    );
    return result;
  };
  ...
  const handleChangeSearchBar = ({ target }) => {
    ...
    const result = clients.filter((client) => {
      ...
      return (
        fullNameClient.includes(term) || fullNamesDependents.includes(term)
      );
    });
  ...
  const checkLateInvoice = (invoices) => {
    ...
    const late = invoices.some(
      (invoice) => new Date(invoice.expiration) < today && !invoice.paid
    );
    return late;
  };
  ...
  useEffect(() => {
    const getUsers = async () => {
      setIsLoading(true);
      const result = await axios.get("/clients/clients");
                ...
                >
                  ...
                  <div className="text-right pr-2">
                    {!client.user.assignments[0] ? (
                      <div className="w-3 h-3 bg-slate-700 rounded-full inline-block">
                        {" "}
                      </div>
                    ) : checkLateInvoice(client.user.invoices) ? (
                      <div className="w-3 h-3 bg-red-700 rounded-full inline-block">
                        {" "}
                      </div>
                    ) : checkPendingInvoice(client.user.invoices) ? (
                      <div className="w-3 h-3 bg-amber-500 rounded-full inline-block">
                        {" "}
                      </div>
                    ) : (
                      <div className="w-3 h-3 bg-green-700 rounded-full inline-block">
                        {" "}
                      </div>
                    )}
                  </div>
