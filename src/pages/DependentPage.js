...
const DependentPage = () => {
  ...
  useEffect(() => {
    const getDependent = async () => {
      setIsLoading(true);
      const result = await axios.get(`/dependents/dependent/${id}`);
      setDependent(result.data);
      const getClient = await axios.get(
        `/users/user/${result.data.assignments.userId}`
      );
              ...
              <div className="p-2 border min-w-[256px] rounded-md mr-2 mb-2">
                ...
                <div>
                  <Link to={`/client/${client.id}`}>
                    <div className="font-bold bg-gray-400 p-2 rounded-md w-full">
